...
def get_grids(filename, grid_size, out_dir):
    i = 0
        ...
        while not (l2) >= max_lon:
            ...
            if not df_grid_.empty:
                ...
                if len(timestamps) >= 1000 and len(vessels) > 3:
                    ...
                    # Saving the file with the same filename in the output directory
                    output_file = os.path.join(out_dir, f"{base_filename}_{i}.csv")
                    df_grid_.to_csv(output_file, index=False)
                    # Plot the trajectory of vessels in this grid
                    plot_trajectory_df(df_grid_)
                    plot_filename = os.path.join(out_dir, f"{base_filename}_trajectory_{i}.png")
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(out_dir):
        os.makedirs(out_dir)
    in_file = "./processed_data/AIS_2022_03_31.csv"
    get_grids(in_file, args.grid_size, out_dir)
